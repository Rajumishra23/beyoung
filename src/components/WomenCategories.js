...
export default function WomenClothingSlider() {
  ...
  return (
    <section className="py-10 sm:py-12 px-4 sm:px-6 md:px-12 bg-white overflow-hidden">
      <h2 className="text-2xl sm:text-3xl md:text-3xl font-extrabold text-center mb-8 sm:mb-10 text-gray-900 tracking-wide uppercase">
      ...
      <div className="relative max-w-7xl mx-auto flex items-center">
        ...
        >
          {WomenCategories.map((item, idx) => (
            <div
              key={idx}
              className="flex-shrink-0 w-[180px] sm:w-[220px] md:w-[250px] bg-white rounded-md overflow-hidden shadow hover:shadow-lg transition"
            >
              ...
              <div className="bg-white border-t border-gray-200 text-black text-center px-3 sm:px-4 py-3 sm:py-4">
                <h3 className="text-sm sm:text-base font-semibold">{item.title}</h3>
                <p className="text-xs sm:text-sm mt-1">{item.description}</p>
                <div className="mt-2 sm:mt-3">
                  <span className="inline-block px-3 py-1 text-xs sm:text-sm font-medium border border-gray-300 rounded-full hover:bg-gray-100 hover:text-orange-500 transition cursor-pointer">
                    + EXPLORE
                  </span>
                </div>
              </div>
